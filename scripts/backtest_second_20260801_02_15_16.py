#!/usr/bin/env python3
"""Backtest Predictjra predictions/results for 2026-08-01, 08-02, 08-15 and 08-16.
...
* Refuse to write partial data: 144 races and all required validation checks must pass.
...
TARGET_DATES = ("2026-08-01", "2026-08-02", "2026-08-15", "2026-08-16")
EXPECTED_RACES_PER_DAY = 36
EXPECTED_TOTAL_RACES = 144
...
REBUILD_VERSION = "predictjra-backtest-20260801-02-15-16-v1-market-memory"
...
DIAGNOSTIC_PATH_DEFAULT = Path("data/backtest_second_diagnostics_20260801_02_15_16.json")
...
def build_fallback_index_detail(
    ...
    This prevents a non-critical display-index issue from blocking all target races.
...
def estimate_popularities(races: dict[str, RaceModel]) -> tuple[dict[str, dict[int, int]], dict]:
    ...
    # Final coefficients for future generalization (trained on all target races).
...
def _selection_context(race: RaceModel, estimated_popularity: dict[int, int]) -> tuple[int, list[int], int]:
    """Return danger, selected horses in ability order, and main horse.

    This duplicates the production selection stage so every second-horse strategy
    compares only the choice of the second axis. Main, danger, selected count and
    the selected horse set remain identical.
    """
    horses = sorted(race.total_internal)
    top3 = [h for h in horses if estimated_popularity[h] <= 3]
    if len(top3) != 3:
    ...
    target_n = prediction_target_count(len(horses))
    selected = sorted(
        [h for h in horses if h != danger],
        key=lambda h: (-race.total_internal[h], race.tie_order.get(h, 999), h),
    )[:target_n]
    return danger, selected, selected[0]


def build_prediction_for_second_strategy(
    race: RaceModel,
    estimated_popularity: dict[int, int],
    strategy: str,
) -> tuple[dict, int]:
    danger, selected, main = _selection_context(race, estimated_popularity)
    ability_candidates = [h for h in selected if h != main]
    if not ability_candidates:
        raise ValueError(f"{race.race_id}: no second-axis candidates")

    if strategy == "current_lowest_expected_popularity":
        pool = ability_candidates
        second = max(
            pool,
            key=lambda h: (estimated_popularity[h], race.total_internal[h], -h),
        )
    elif strategy == "ability_rank_2":
        second = ability_candidates[0]
    elif strategy.startswith("ability_top") and strategy.endswith("_lowest_expected_popularity"):
        # Example ability_top4_* means: among ability ranks 2-4 (main is rank 1),
        # take the horse expected to be least popular. This keeps a minimum ability
        # floor while preserving the original value-seeking intent.
        m = re.match(r"ability_top(\d+)_lowest_expected_popularity$", strategy)
        if not m:
            raise ValueError(strategy)
        top_n = int(m.group(1))
        pool = ability_candidates[:max(1, top_n - 1)]
        second = max(
            pool,
            key=lambda h: (estimated_popularity[h], race.total_internal[h], -h),
        )
    ...
    opponents = [h for h in selected if h not in {main, second}]
    opponents.sort(
        key=lambda h: (-race.total_internal[h], race.tie_order.get(h, 999), h)
    )
    return {"axes": [main, second], "opponents": opponents}, danger
...
def backtest_second_strategies(
    races: dict[str, RaceModel],
    estimated_pops: dict[str, dict[int, int]],
) -> dict:
    strategies = [
        "current_lowest_expected_popularity",
        "ability_rank_2",
        "ability_top3_lowest_expected_popularity",
        "ability_top4_lowest_expected_popularity",
        "ability_top5_lowest_expected_popularity",
        "ability_top6_lowest_expected_popularity",
        "ability_top7_lowest_expected_popularity",
    ]
    out = {}
    for strategy in strategies:
        total_hits = total_return = total_stake = 0
        second_top3 = 0
        second_starters = 0
        daily = {}
        hit_races = []
        for rid, race in sorted(races.items(), key=lambda kv: (kv[1].date, kv[0])):
            prediction, _danger = build_prediction_for_second_strategy(
                race, estimated_pops[rid], strategy
            )
            result, _ = build_result(race)
            ...
            second = int(prediction["axes"][1])
            starters_top3 = {
                int(no)
                for group in result["places"][:3]
                for no in group
            }
            if second in race.actual_popularity:
                second_starters += 1
                second_top3 += int(second in starters_top3)

            d = daily.setdefault(race.date, {"races": 0, "hits": 0, "return": 0, "stake": 0})
            d["races"] += 1
            d["hits"] += int(hit)
            d["return"] += payout_return
            d["stake"] += stake
            total_hits += int(hit)
            total_return += payout_return
            total_stake += stake
            if hit:
                hit_races.append({"date": race.date, "raceId": rid, "return": payout_return})

        for d in daily.values():
            d["recoveryRate"] = round(d["return"] / d["stake"] * 100, 1) if d["stake"] else 0.0
        out[strategy] = {
            "races": len(races),
            "hits": total_hits,
            "return": total_return,
            "stake": total_stake,
            "recoveryRate": round(total_return / total_stake * 100, 1) if total_stake else 0.0,
            "secondAxisTop3Rate": round(second_top3 / second_starters * 100, 1) if second_starters else 0.0,
            "daily": daily,
            "hitRaces": hit_races,
        }
    return out
...
def main() -> int:
    ...
    parser.add_argument("--audit-path", default="data/second_strategy_backtest_20260801_02_15_16.json", type=Path)
    parser.add_argument("--pop-model-path", default="data/backtest_popularity_model_20260801_02_15_16.json", type=Path)
    ...
    audit["secondHorseStrategyBacktest"] = backtest_second_strategies(
        races, estimated_pops
    )

    pop_model = {
        "version": "predictjra-backtest-popularity-calibration-20260801-02-15-16-v1-market-memory",
