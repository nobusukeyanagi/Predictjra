...
## v5 想定人気 — 時系列・条件別 Market Memory

想定人気は、現在レースのオッズ・実人気を一切使わず、対象レースより前に確定していた情報だけから市場評価を推定します。

- 検証方式を leave-one-race-out から **expanding-window（過去→未来）** に変更。同日および未来日の確定人気は教師に使いません。
- 馬自身の直近人気だけでなく、人気の長期平均・上昇/下降トレンド・安定度を利用します。
- 芝/ダート/障害、近似距離（±300m）の過去市場評価を別特徴量として扱います。
- 騎手・調教師は全体市場評価に加え、対象路面での市場評価を縮約平均で保持します。
- クラス昇降、前走からの日数、斤量変化、近走成績を補助特徴量として利用します。
- 初期学習期間は固定のMarket Memoryブレンドを使い、十分な過去教師レースが蓄積した後にridgeモデルへ段階的に寄せます。
- Rebuild後の `data/rebuild_audit.json` に `sameDayLabelsUsed=false` / `futureDateLabelsUsed=false` と検証結果を保存します。

前回検証で得た約73.7%のTop3一致率を再現するための構造変更であり、特定レースだけを上書きする補正や当日オッズによる補正はありません。実際の採用判定は、最新キャッシュで `all / validate / auto` を実行した際の `meanTop3OverlapRate` を正とします。
...
過去の出走情報・結果などの事実データは `data/history_cache/` に保存し、通常は `cache_policy = auto` で再利用します。2026年1月以降は確定result/payoutを実際の日付で列挙し、保存済みrace cardがない日については、確定resultから **レース前に確定していた固定項目だけ**（馬番・馬名・性齢・斤量・騎手・調教師・芝/ダート・距離・各ID等）を抽出してrace cardを再構成します。対象レースの着順・時計・着差・実人気・当日単勝オッズ・馬体重は予想入力へ渡しません。対象馬の過去走は2023年以降の確定resultから収集し、各対象レースより前の日付だけを現行予想ロジックへ渡します。

キャッシュ作成はfail-closedです。各開催の1R〜12R、開催日番号の連続性、result/payoutのrace_id、1〜3着と三連単払戻、保存済みcardとresultのレース集合、禁止列の除去を検査し、1件でも不整合があれば新キャッシュを採用しません。v4へ切り替える最初の1回は **`all / validate / refresh`** で完全再取得・検証し、正常終了後に採用するときだけ `all / apply / auto` を実行してください。旧予想値は再利用しません。
...
3. 通常は `Rebuild historical predictions` を **`all / validate / auto`** で手動実行する（2026年1月Backfillを初導入する1回だけは **`all / validate / refresh`**）
